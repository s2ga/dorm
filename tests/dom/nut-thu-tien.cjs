...
(async () => {
  ...
  ok('Cặp gạt hiện CẢ HAI trạng thái cạnh nhau', await page.locator('.seg-thu').count() > 0);
  ok('Trạng thái đang đúng được tô và KHÔNG bấm được', await page.locator('.seg-thu .seg.on:not(button)').count() > 0);

  if (n > 0) {
    const dau = nut.first();
    const truoc = (await dau.textContent() || '').trim();
    ok('Nút bên cạnh là hành động chuyển sang trạng thái kia', /Đã thu|Chưa thu/.test(truoc), truoc);

    // Chỉ thử trên phiếu đang CHƯA thu để khỏi đụng dữ liệu đã chốt.
    if (/Đã thu/.test(truoc)) {
      ...
      ok('Phiếu đã thu thì KHOÁ — không còn nút sửa/xoá/tính lại',
        await page.locator(`tr:has([data-args='[${id},"pending"]']) [data-act="invoiceForm"]`).count() === 0);
      if (sau > 0) {
