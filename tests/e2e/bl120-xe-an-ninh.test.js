  ...
  async run(t) {
    ...
    try {
      ...
      const hv1 = await t.api('POST', '/api/students', T, {
        name: P + ' Mot', code: P + '_M1', gender: 'male', birth_date: '2004-05-06', room_id: R1, check_in_date: lui(90), rental_type: 'ghep', confirm_overload: true,
      });
      t.eq('Dựng HV1 đang ở', hv1.status, 201, loi(hv1));
      const hv2 = await t.api('POST', '/api/students', T, {
        name: P + ' Hai', code: P + '_M2', gender: 'male', birth_date: '2004-05-06', room_id: R1, check_in_date: lui(60), rental_type: 'ghep', confirm_overload: true,
      });
