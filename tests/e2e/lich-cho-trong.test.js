...
module.exports = {
  ...
  async run(t) {
    ...
    const ridWhole = (await t.db.query(
      ...
      [P + '_W'])).rows[0].id;
    // BL-117: ngày THẬT (check_in/out_date) chỉ có khi đã xác nhận; tương lai là DỰ KIẾN (planned_*).
    const hv = async (ten, vaoThat, duKienVao, duKienRa) => (await t.db.query(
    ...
    await hv('A', ngay(-30), null, ngay(10));   // đang ở thật, lịch rời ngày +10
    await hv('B', null, ngay(5), null);         // đã đặt chỗ, dự kiến vào ngày +5
    await hv('C', ngay(-30), null, null);       // đang ở
