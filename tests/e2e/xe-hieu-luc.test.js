...
module.exports = {
  ...
  async run(t) {
    ...
    // ── Có LỊCH trả phòng (BL-117: planned_check_out, ngày tương lai) thì to_date theo lịch ──
    // Ngày tương đối, không ghi cứng: ngày cứng trôi vào quá khứ là máy chủ từ chối tạo xe
    // ("đã trả phòng") rồi 3 ca sau đỏ dây chuyền vì id undefined.
    const lichRa = (() => { const d = new Date(); d.setDate(d.getDate() + 30); return d.toISOString().slice(0, 10); })();
    await t.db.query('UPDATE students SET planned_check_out=$1 WHERE id=$2', [lichRa, sid]);
    const e = await t.api('POST', '/api/vehicles', T, { student_id: sid, plate: P + '_B3' });
    t.eq('Tạo xe cho người có lịch trả → 201', e.status, 201, `HTTP ${e.status} ${e.json && e.json.error || ''}`);
    t.eq('Có lịch trả phòng → to_date lấy theo lịch', d10(e.json.to_date), lichRa,
