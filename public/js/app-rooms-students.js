...
function roomDetail(id) {
  ...
  datManChiTiet(roomDetail, id);   // form con lưu xong quay về đúng phòng này (luuXongVeLai)
  const dsDangO = ST.students.filter(s => s.room_id === id && isOccupying(s));
  const dsSapVao = ST.students.filter(s => s.room_id === id && ['upcoming', 'cho_vao'].includes(liveStatus(s)));
  ...
  const dong = s => `<tr data-act="studentDetail" data-args='[${s.id}]' role="button" tabindex="0" title="Xem chi tiết học viên">
    ...
    <td>${s.check_in_date ? fmtDate(s.check_in_date) : s.planned_check_in ? `${fmtDate(s.planned_check_in)}<div class="sub2">dự kiến</div>` : '<span class="muted">—</span>'}</td>
    <td>${s.check_out_date ? fmtDate(s.check_out_date) : s.planned_check_out ? `${fmtDate(s.planned_check_out)}<div class="sub2">dự kiến</div>` : '<span class="muted">—</span>'}</td>
...
function stuSortVal(s) {
  switch (stuSort.key) {
    ...
    case 'checkin': return (s.check_in_date || s.planned_check_in || '').slice(0, 10);
    default: return 0;
  }
}
...
function viewStudents() {
  ...
  // Cùng định nghĩa với 2 ô "hôm nay" ở Tổng quan (app-exec-dashboard.js): theo LỊCH, chưa xác nhận
  if (stuFilter === 'checkin_today') list = list.filter(s => !s.check_in_date && s.planned_check_in && String(s.planned_check_in).slice(0, 10) === today());
  if (stuFilter === 'checkout_today') list = list.filter(s => s.check_in_date && !s.check_out_date && s.planned_check_out && String(s.planned_check_out).slice(0, 10) === today());
...
function roomOptions(sel, gender) {
  ...
  // Xếp được vào MỌI loại phòng cùng giới — kể cả phòng thuê NGUYÊN PHÒNG (owner chốt 28/08:
  // BQL vẫn quản người ra vào phòng thuê nguyên; server sẽ cảnh báo + bắt xác nhận "ở chung theo HĐ").
  const rooms = nguon.filter(r => !gender || r.gender === gender);
...
async function studentForm(id) {
  ...
  openModal(`
    ...
    <div class="mb">
      ...
      ${nhomForm(IC.bed, 'Thông tin phòng', `
        <div class="grid2">
    ...
    : '<input id="f_out">'}</div>
      ...
      ${nhomForm(IC.fileText, 'Thông tin hợp đồng', `
        ...
        <div class="field"><label>Tình trạng HĐ</label><select id="f_cstatus">
  ...
  // BL-117: chưa xác nhận thì ô này là NGÀY DỰ KIẾN (máy chủ tự ghi vào planned_check_in); đã xác nhận
  // thì là ngày thật. Form không bao giờ tự xác nhận vào/ra.
    ...
    : '<p class="muted" style="margin:0 0 6px;font-size:12px">Chưa có.</p>'}</div>`).join('')}
      ...
      <div class="panel"><div class="hd"><h2 style="font-size:14px">${IC.bike} Xe (${vehicles.length})</h2>
        ...
        ${vehicles.length ? vehicles.map(v => `<div style="padding:6px 0;border-bottom:1px solid var(--line)">
          ...
          <div class="sub2">${IC.calendar} ${fmtDate(v.from_date)} → ${v.to_date ? fmtDate(v.to_date) : 'còn gửi'}${v.note ? ' · ' + esc(v.note) : ''}</div>
...
/* Xe */
// Về lại đúng màn vừa mở form: Dịch vụ → Gửi xe, hoặc hồ sơ học viên.
function veManXe(studentId) {
  if (ST.view === 'services') { svcTab = 'parking'; viewServices(); }
  else if (studentId) studentDetail(studentId);
}
// Đổi học viên ở form thêm xe -> lấy lại ngày nhận/trả phòng của người mới làm khoảng hiệu lực.
function onVehicleStudent() {
  const s = studentById(+this.value) || {};
  attachDate(el('v_from'), (s.check_in_date || '').slice(0, 10));
  attachDate(el('v_to'), (s.check_out_date || '').slice(0, 10));
}
function vehicleForm(vid, studentId) {
  const dsXe = window._detailVehicles || [];
  const cu = vid ? dsXe.find(x => x.id === vid) : null;
  const sid = studentId || (cu && cu.student_id) || 0;
  const s = studentById(sid) || window._detailStudent || {};
  const v = cu || { plate: '', vehicle_type: '', sticker: '', note: '', from_date: s.check_in_date, to_date: s.check_out_date };
...
async function saveVehicle(vid, studentId) {
  ...
  const body = {
    student_id: sid, plate: el('v_plate').value.trim(), vehicle_type: el('v_type').value.trim(),
  ...
  await refreshCache(); closeModal(); toast('Đã lưu xe'); veManXe(sid);
}
...
async function doKhoaHoSo(id) {
  ...
  await refreshCache(); await luuXongVeLai(veLaiNen);
  toast(`Đã khoá ${s.name || 'học viên'} — ${(r && r.lock_reason) || ''}`);
}
...
async function restoreStudentAndReload(id) {
  ...
  await refreshCache(); await luuXongVeLai(veLaiNen); toast('Đã mở khoá học viên');
}
...
async function saveDeposit(id) {
  ...
  await refreshCache(); await luuXongVeLai(veLaiNen); toast('Đã ghi nhận cọc');
}
