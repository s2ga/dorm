...
async function viewInvoices() {
  ...
  const coCoc = all.some(i => +i.deposit_charge);   // cột Cọc chỉ chiếm chỗ ở kỳ thật sự có thu cọc
...
// Số phiếu lấy từ máy chủ: gọi không kèm confirm -> 400 kèm would_update, là con số sẽ đổi thật.
async function thuCaKyForm() {
  let n = (_invAll || []).filter(i => i.status !== 'paid').length;
  try { await API.markPaid(invMonth, false); } catch (e) { if (e && e.data && e.data.would_update != null) n = e.data.would_update; }
  openModal(`
    <div class="mh"><h3>${IC.checkCircle} Đã thu cả kỳ ${monthLabel(invMonth)}</h3><button class="x" aria-label="Đóng" data-act="closeModalNgay">×</button></div>
    <div class="mb">
      <p style="margin:0 0 10px">Chuyển <strong>${n} phiếu</strong> đang <em>Chưa thu</em> của kỳ ${monthLabel(invMonth)} sang <strong>Đã thu</strong>, ngày thu = hôm nay (${fmtDate(today())}). Phiếu đã thu rồi giữ nguyên.</p>
      <div class="hint">${IC.alert}<span><strong>Không hoàn tác hàng loạt được</strong> — muốn sửa lại phải mở khoá từng phiếu. Chỉ bấm khi tiền của <strong>tất cả ${n} phiếu</strong> đã thật sự nhận đủ.</span></div>
      <label class="check" style="margin-top:10px"><input type="checkbox" id="tck_ok"> Tôi xác nhận đã thu đủ tiền của cả ${n} phiếu</label>
    </div>
    <div class="mf"><button class="btn" data-act="closeModalNgay">Hủy</button><button class="btn pri" data-act="doThuCaKy">${IC.checkCircle} Đánh dấu đã thu (${n})</button></div>`);
}
...
async function electricForm(thangMo) {
  openModal(`<div class="mb"><div class="spinner"></div></div>`, 'x');   // xem ghi chú ở generateForm
  const m = typeof thangMo === 'string' && /^\d{4}-\d{2}$/.test(thangMo) ? thangMo : elecMonth;
  await renderElectricForm(m);
}
async function moChotGiuaKy(month) {
  closeModalNgay();
  await electricForm(month);
  const o = el('chot_giua_ky');
...
function chotGiuaKyHTML(month, reads, rooms) {
  ...
  // data-mrkey là khoá BỀN qua lần vẽ lại (chỉ số dòng thì đổi khi danh sách rút ngắn).
  const dongThieu = thieu.map((m, i) => {
    const ngay = String(m.ngay_can_nhap || m.to_date).slice(0, 10);
    const khoa = `${m.room_id}|${ngay}|${m.student_id || 0}`;
    return `
    <tr>
...
// Vẽ lại RIÊNG khối chốt giữa kỳ, giữ nguyên bảng chỉ số cuối kỳ phía trên và những ô đang gõ dở.
async function veLaiChotGiuaKy(month) {
  const box = el('chot_giua_ky');
  if (!box) return;
  const giu = giuChotGiuaKy();
...
function giuChotGiuaKy() {
  const m = {};
  document.querySelectorAll('#modal input[data-mrkey]').forEach(i => { if (i.value.trim() !== '') m[i.dataset.mrkey] = i.value; });
  return m;
}
...
// meter.CheckRead so với lần chốt liền trước/liền sau.
async function luuTatCaChotGiuaKy() {
  const o = [...document.querySelectorAll('#modal input[data-mrkey]')].filter(i => i.value.trim() !== '');
  if (!o.length) return toast('Chưa nhập ô chỉ số nào', 'err');
  const xau = o.filter(i => isNaN(+i.value) || +i.value < 0);
  if (xau.length) return toast(`${xau.length} ô không phải số không âm — sửa lại rồi lưu`, 'err');
  o.sort((a, b) => a.dataset.mrkey.split('|')[1].localeCompare(b.dataset.mrkey.split('|')[1]));
  let ok = 0, hoaDon = 0; const loi = [];
  for (const i of o) {
    const [roomId, date, studentId] = i.dataset.mrkey.split('|');
    try {
      const r = await API.saveMeterRead({ room_id: +roomId, date, reading: +i.value, student_id: +studentId || undefined });
      ok++; hoaDon += r.recalculated || 0;
...
async function luuGiamPct(invoiceId, sid) {
  const body = {};
  GIAM_O.forEach(([k]) => { const o = el('g_' + k); if (o) body[k] = Math.max(0, Math.min(100, +o.value || 0)); });
  const r = await guard(() => withOverloadConfirm(ok => API.updateStudent(sid, { ...body, confirm_overload: ok })));
  if (r === null) return;
...
function duyetTaiKhoanForm(id, mode) {
  const u = (window._usrCache || []).find(x => x.id === id);
  if (!u) return;
  // mode 'hocvien': tài khoản đã LỠ gán vai nhân viên — chỉ còn một việc là ghép hồ sơ học viên.
  // Máy chủ (approve-student) nhận mọi vai trừ admin; đổi vai xong tự thu hồi vé đăng nhập cũ.
  const chuyen = mode === 'hocvien';
