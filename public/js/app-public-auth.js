...
async function renderLogin() {
  ...
  API.ssoConfig().then(c => {
    if (!(c && c.enabled)) { const u = el('lg_user'); if (u) u.focus(); return; }
    el('lgSso').style.display = '';
    el('loginForm').hidden = true;
    el('lgSub').textContent = 'Dùng tài khoản Microsoft của công ty để đăng nhập.';
  }).catch(() => { const u = el('lg_user'); if (u) u.focus(); });
...
// Mở form tài khoản do Ban quản lý cấp khi màn đang chỉ hiện Microsoft (nhân viên / học viên chưa có Microsoft).
function lgMoNoiBo() {
  const f = el('loginForm'); if (!f) return;
  f.hidden = false;
  const alt = el('lgAlt'); if (alt) alt.hidden = true;
  const u = el('lg_user'); if (u) u.focus();
}
...
// BL-117: check_in_date/check_out_date là ngày THẬT (chỉ có sau bước xác nhận); planned_* là dự kiến.
// Trạng thái đi theo ngày thật; dự kiến chỉ sinh ra "sắp vào / sắp trả" và hai trạng thái CHỜ XÁC NHẬN
// khi ngày dự kiến đã tới mà chưa ai xác nhận. Không có ngày vào thật = chưa bao giờ "đang ở".
const _d10 = v => (v ? String(v).slice(0, 10) : '');
function liveStatus(s) {
  const t = today(), ci = _d10(s.check_in_date), co = _d10(s.check_out_date);
  const pi = _d10(s.planned_check_in), po = _d10(s.planned_check_out);
  if (co) return 'left';
  if (!ci) {
    if (!pi) return 'unset';
    return pi > t ? 'upcoming' : 'cho_vao';
  }
  if (po) return po > t ? 'leaving' : 'cho_ra';
  return 'staying';
}
const STATUS_INFO = {
  unset: ['Chưa có lịch vào', 'gray'], upcoming: ['Sắp vào', 'blue'], cho_vao: ['Chờ xác nhận vào', 'amber'],
  staying: ['Đang ở', 'green'], leaving: ['Sắp trả', 'amber'], cho_ra: ['Chờ xác nhận trả', 'amber'], left: ['Đã trả', 'gray'],
};
...
// Ở ngắn hạn = DỰ ĐỊNH ở bao lâu -> lấy ngày thật nếu có, không thì ngày dự kiến.
function isShortStay(s) {
  const ci = _d10(s.check_in_date) || _d10(s.planned_check_in), co = _d10(s.check_out_date) || _d10(s.planned_check_out);
  if (!co || !ci) return false;
  const d = (Date.parse(co) - Date.parse(ci)) / DAY_MS;
...
function nextDepartureDate(s) {
  const t = today();
  const cands = [s.expected_departure, (DEPARTURE_REASONS.includes(s.checkout_reason) ? (s.planned_check_out || s.check_out_date) : null)]
