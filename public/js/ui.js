...
function closeModalNgay() {
  const dangMo = el('overlay').classList.contains('show');
  _formLucMo = null; _lopModal = []; _manChiTiet = null;
...
// Màn chi tiết gọi datManChiTiet(hàm, ...args) khi vẽ; form con lưu xong gọi luuXongVeLai(lamTuoiNen)
// -> vẽ lại màn chi tiết bằng dữ liệu mới, người dùng đứng nguyên chỗ cũ. Không đứng trong màn chi
// tiết nào -> đóng modal + làm tươi màn nền, đúng hành vi cũ.
let _manChiTiet = null;
function datManChiTiet(fn, ...args) { _manChiTiet = { fn, args }; }
async function luuXongVeLai(lamTuoiNen) {
  const ct = _manChiTiet;
  window._dangLuu = true;
  try {
    if (ct && el('overlay').classList.contains('show')) {
      _lopModal.length = 0;   // ảnh chụp các lớp cũ đã lỗi thời — màn chi tiết vẽ mới thành lớp gốc
      try { await ct.fn(...ct.args); } catch (e) { closeModalNgay(); }
    } else {
      closeModalNgay();
    }
    if (typeof lamTuoiNen === 'function') await lamTuoiNen();
  } finally { window._dangLuu = false; }
}
