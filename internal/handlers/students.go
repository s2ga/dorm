...
var studentsDateFields = []string{"birth_date", "check_in_date", "check_out_date", "planned_check_in", "planned_check_out", "contract_date", "deposit_date", "class_start_date", "expected_departure", "checkout_notice_date"}
...
// studentsChiSuaThongTin: lần lưu KHÔNG đổi phòng (người đang ở: cả giới tính + loại thuê) -> chỉ là
// sửa thông tin, đừng bắt qua luật xếp phòng.
func studentsChiSuaThongTin(cur map[string]interface{}, b map[string]interface{}) bool {
	...
	cungPhong := (moi == nil && cuID == nil) || (moi != nil && cuID != nil && *moi == *cuID)
	if !cungPhong {
		return false
	}
	...
	return studentsJSString(b["gender"]) == studentsJSString(cur["gender"]) &&
		studentsJSString(b["rental_type"]) == studentsJSString(cur["rental_type"])
}
...
func (h *Handlers) CreateStudent(c *gin.Context) {
	...
	txErr := h.DB.WithTx(ctx, func(tx pgx.Tx) error {
		...
		student = st
		sid := intFromDB(st["id"])
		// BL-117: ngày còn ở TƯƠNG LAI = dự kiến, chưa ai xác nhận -> chuyển sang planned_*, không mở lượt
		// ở, không ghi nhật ký "vào ở". Ngày hôm nay/quá khứ do admin nhập tay = đã xác nhận.
		vaoThat := checkIn <= today
		if !vaoThat {
			if _, e := tx.Exec(ctx, "UPDATE students SET planned_check_in=check_in_date, check_in_date=NULL WHERE id=$1", sid); e != nil {
				return e
			}
			student["planned_check_in"], student["check_in_date"] = checkIn, nil
		}
		if checkOut != "" && checkOut > today {
			if _, e := tx.Exec(ctx, "UPDATE students SET planned_check_out=check_out_date, check_out_date=NULL WHERE id=$1", sid); e != nil {
				return e
			}
			student["planned_check_out"], student["check_out_date"] = checkOut, nil
		}
		// (CCCD upload BỎ — CHƯA port S3; bản ghi vẫn được tạo với cccd_* = null)
		if studentsJSTruthy(b["room_id"]) && vaoThat {
...
func (h *Handlers) UpdateStudent(c *gin.Context) {
	...
	coThat := studentsSlice10(studentsJSString(cur["check_out_date"]))
	lichCu := studentsSlice10(studentsJSString(cur["planned_check_out"]))
	...
	cols := `code=$1, name=$2, gender=$3, phone=$4, id_card=$5, birth_date=$6, class_name=$7, room_id=$8,
      ...
      planned_check_out=CASE WHEN check_out_date IS NOT NULL THEN planned_check_out ELSE $27::date END`
...
func (h *Handlers) StudentCheckin(c *gin.Context) {
	...
	// (và cả kỳ dự kiến cũ nếu vào lệch tháng).
...
func (h *Handlers) UpdateCheckoutDate(c *gin.Context) {
	...
	// Ngày bàn giao an ninh (checkout_actual_date) phải chạy theo, không thì màn Bảo trì còn hiện ngày cũ.
	sets := []string{"check_out_date=$1",
		"checkout_actual_date=CASE WHEN checkout_confirmed_at IS NOT NULL THEN $1::date ELSE checkout_actual_date END"}
	args := []interface{}{d}
