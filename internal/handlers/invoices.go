...
func (h *Handlers) GenerateInvoices(c *gin.Context) {
	...
	txErr := h.DB.WithTx(ctx, func(tx pgx.Tx) error {
		...
		tenRoi := map[int]string{} // student_id -> tên cho câu cảnh báo "thiếu mốc chốt" — phải nói rõ AI
		khoaRoi := map[int]bool{}
		rsRows, err := tx.Query(ctx,
			`SELECT rs.room_id, rs.student_id, rs.from_date AS from, rs.to_date AS to,
			        COALESCE(s.name,''), (s.deleted_at IS NOT NULL)
		...
		for rsRows.Next() {
			...
			var ten string
			var khoa bool
			if err := rsRows.Scan(&roomID, &sid, &from, &to, &ten, &khoa); err != nil {
				rsRows.Close()
				return err
			}
			if ten != "" {
		...
		for rid, stays := range staysByRoom {
			...
			// Phòng thuê NGUYÊN: trọn công-tơ lên phiếu người ký HĐ, không chia đầu người — không cần
			// mốc giữa kỳ khi có người rời (owner 03/09: điện phòng nguyên chốt thẳng với trưởng phòng).
			if roomsCache[rid].roomType != "whole" {
		...
		// Dọn phiếu CHƯA THU của người rời TRƯỚC kỳ: rỗng, hoặc còn tính phòng/nước/dịch vụ/giặt/xe cho
		// tháng không ở. Chỉ-điện thì giữ (điện thu lùi kỳ = truy thu thật); có khoản khác/cọc tay thì
		// không đụng; đã thu thì để data-health chỉ mặt. Phiếu xoá mềm được lập lại khi đủ chỉ số điện.
		dRows, err := tx.Query(ctx,
			`UPDATE invoices i SET deleted_at = now()
			  ...
			  WHERE s.id = i.student_id AND i.month = $1 AND i.deleted_at IS NULL AND i.status <> 'paid'
			    ...
			    AND (i.electric_charge = 0 OR i.room_charge <> 0 OR i.water_charge <> 0
			         OR i.service_charge <> 0 OR i.washing_charge <> 0 OR i.parking_charge <> 0)
		...
		for _, s := range students {
			...
			// Người rời TRƯỚC kỳ mà kỳ này không còn khoản nào (điện truy thu = 0): không phát phiếu 0đ;
			// phiếu cũ chưa thu dọn đi. Lần dọn trước vòng tính không bắt được ca này vì lúc đó phiếu
			// còn mang tiền phòng cũ — chỉ sau khi tính lại mới biết nó rỗng.
			if s.checkOut != "" && s.checkOut < mStart && inv.Total == 0 && (!hasDup || dup.other == 0 && dup.coc == 0) {
				if hasDup && !dup.daXoa {
					if _, err := tx.Exec(ctx, "UPDATE invoices SET deleted_at=now() WHERE id=$1", dup.id); err != nil {
						return err
					}
